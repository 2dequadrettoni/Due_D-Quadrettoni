...
public partial class Door : UsableObject {
	...
	private		void	Start() {
		...
		if ( transform.childCount > 0 ) {
			...
			if ( !pAnimator ) {
				GLOBALS.UI.ShowMessage( "Invalid Door", "A door object has not sprite with animator component", GameManager.Exit );
				return;
			}

		}
		else {
			GLOBALS.UI.ShowMessage( "Invalid Door", "A door object has not sprite as child", GameManager.Exit );
			Destroy( gameObject );
		}

		pSpriteRender	= transform.GetChild( 0 ).GetComponent<SpriteRenderer>();

		transform.Find( "Direction" ).GetComponent<SpriteRenderer>().enabled = false;
...
//		bStartClosed = bClosed;
	...
	private	void			Close() {
		...
		AudioManager.Play( "Door_Closing" );
